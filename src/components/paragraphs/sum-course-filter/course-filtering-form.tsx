...
import {useSearchBox, useRefinementList, useClearRefinements, useCurrentRefinements, useInfiniteHits, usePagination} from "react-instantsearch"
    ...
    <div className="grid grid-cols-12 gap-12">
      <div className="col-span-12 flex flex-col md:col-span-4 xl:col-span-3">
        ...
        >
          ...
          <div className="mb-10">
            ...
            <div className="relative mt-4">
              ...
              >
                ...
                <MagnifyingGlassIcon
                  width={40}
                  className="block rounded-full bg-digital-red p-3 text-white"
                />
              </button>
            </div>
          </div>
          ...
          <RefinementInput
            attribute="sum_course_interest"
            label="Filter by Interest Area"
            accordionExpanded={true}
          />

          <RefinementInput
            attribute="sum_course_format"
            label="Filter by format"
          />

          <RefinementInput
            attribute="sum_course_availability"
            label="Filter by availability"
          />

          <RefinementInput
            attribute="sum_course_population"
...
const RefinementInput = ({attribute, label, accordionExpanded = false, useRefinementProps}: {attribute: string; label: string; accordionExpanded?: boolean; useRefinementProps?: Omit<RefinementListConnectorParams, "attribute">}) => {
    ...
    <fieldset className="rs-mb-1">
      <legend className="w-full border-t border-black">
        <H3 className="my-5 pb-0 text-18">
          <button
            {...buttonProps}
            className="flex w-full items-center justify-between hocus:underline"
          >
            {label}
            <ChevronDownIcon
              width={20}
      ...
      <div {...panelProps}>
        {refinementOptions.map(refinementOption => (
          <label
            ...
            className="mb-5 flex items-center gap-5 last:mb-0"
          >
            <input
...
const HitList = () => {
  ...
  return (
    <div>
      <H2
        className="rs-ml-1 text-m0 font-normal"
        aria-live="polite"
        aria-atomic
      >
