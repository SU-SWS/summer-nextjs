...
import {NodeStanfordPage, NodeStanfordPageSuPageBannerUnion, NodeStanfordPageSuPageComponentsUnion} from "@lib/gql/__generated__/drupal.d"
...
const StanfordPagePage = ({node, ...props}: Props) => {
  ...
  const fullWidthComponents: NodeStanfordPageSuPageComponentsUnion["__typename"][] = ["ParagraphSumTestimonial", "ParagraphSumCarousel", "ParagraphSumCourseFilter", "ParagraphSumPillBanner"]
  const pageTitleBanners: NodeStanfordPageSuPageBannerUnion["__typename"][] = ["ParagraphStanfordPageTitleBanner", "ParagraphSumArcBanner", "ParagraphSumTopBanner"]

  const fullWidth = node.layoutSelection?.id === "stanford_basic_page_full" || !!node.suPageComponents?.find(component => fullWidthComponents.includes(component.__typename))
