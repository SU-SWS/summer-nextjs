...
const SumCalculatorParagraph = ({paragraph, ...props}: Props) => {
  ...
  costPerStudentType.set(
    "undergraduate",
    new Map(paragraph.sumCalcUgUnitCost.map(cost => [cost.first as number, cost.second as number]))
  )
  costPerStudentType.set(
    "highschool",
    new Map(paragraph.sumCalcHighUnitCost.map(cost => [cost.first as number, cost.second as number]))
  )
  costPerStudentType.set(
    "graduate",
    new Map(paragraph.sumCalcGradUnitCost.map(cost => [cost.first as number, cost.second as number]))
  )
