...
const Home = async () => {
  const { entity, error } = await getEntityFromPath<NodeStanfordPage>(
    "/",
    isPreviewMode(),
  );
  ...
  return (
    <article>
      {entity.suPageBanner?.__typename ===
...
export const generateMetadata = async (): Promise<Metadata> => {
  const siteSettingsConfig = await getConfigPage<StanfordBasicSiteSetting>(
    "StanfordBasicSiteSetting",
  );
  ...
  if (metadata.openGraph?.title)
    metadata.openGraph.title =
      siteSettingsConfig?.suSiteName || metadata.openGraph.title;
  return metadata;
};
